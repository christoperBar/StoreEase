...
struct VDKComboBox: NSViewRepresentable {
    var itemProducts: [ProductItem]
    
    @Binding var text: String
    
    var onSelect: (ProductItem) -> Void
    
    func getProductNames(from items: [ProductItem]) -> [String] {
        return items.map { $0.name }
    }
    
    func findProduct(byName name: String) -> ProductItem? {
            return itemProducts.first { $0.name == name }
    }
    
    func filteredProductNames(from items: [ProductItem]) -> [String] {
        if text.isEmpty {
            return items.map { $0.name }
        }
